...
function generateFallbackReply(message, cards, lifeCard) {
  ...
  if (/평생|오래|처음|막막|모르|추천|어떤 카드|뭐가 좋|골라|다양/.test(msg)) {
    return {
      text: `"BNK 영원카드"를 추천드려요! 어릴 때부터 노년까지 한 장으로, AI가 나이·소비를 분석해 혜택을 자동으로 바꿔주고 오래 쓸수록 적립률이 올라가요 (카페 20%→최대 28%). 지금 바로 확인해보세요.`,
      recommendedCards: life,
    }
  }
  if (/카페|커피|배달|편의점|구독|월급|사회초년/.test(msg)) {
    return {
      text: `카페·배달·구독을 자주 쓰신다면 "BNK 영원카드"가 딱이에요. 카페 20%·배달 10% 적립에, 오래 쓸수록 혜택이 자라요. 20대 사회초년생에게 가장 인기예요!`,
      recommendedCards: life,
    }
  }
  if (/마트|주유|가족|아이|육아/.test(msg)) {
    return {
      text: `가정 지출이 많으시군요! "BNK 영원카드"는 생애단계가 바뀌면 마트·주유·교육 혜택으로 자동 전환돼요. 한 장으로 평생 쓰는 카드예요.`,
      recommendedCards: life,
    }
  }
  if (/체크/.test(msg)) {
    const checks = cards.filter(c => c.type === '체크카드')
    return { text: `연회비 없는 체크카드를 찾으시는군요! 그중에서도 평생 쓰는 "BNK 영원카드"(체크·연회비 무료)를 가장 추천해요.`, recommendedCards: life.concat(checks.filter(c => c.name !== LIFE_CARD_NAME)).slice(0, 3) }
  }
  if (/신청|발급|만들/.test(msg)) {
    return { text: `"BNK 영원카드" 상세 페이지에서 "이 카드 만들기"로 바로 신청하실 수 있어요. 신분증 인증 후 발급돼요!`, recommendedCards: life }
  }
  if (/안녕|하이|hello/.test(msg)) {
    return { text: `안녕하세요! BNK 카드 AI 상담사 피카예요 :) 어떤 소비를 자주 하시는지 알려주시면 딱 맞는 카드를 찾아드릴게요!`, recommendedCards: [] }
  }
  return {
    text: `어떤 곳에 소비를 많이 하시나요? 카페·배달이 많으면 "BNK 영원카드"처럼 소비에 맞춰 자라는 카드를 추천해드릴 수 있어요.`,
    recommendedCards: life,
  }
}
